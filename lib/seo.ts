...
export function generateProjectMetadata(project: any) {
  const baseUrl =
    process.env.NEXT_PUBLIC_BASE_URL || "https://www.Sohails.online";
  const url = `${baseUrl}/projects/${project.slug}`;

  return {
    title: `${project.name} | Syed Sohail Portfolio`,
...
export function generateStructuredData(
  ...
  if (type === "website") {
    return {
      "@context": "https://schema.org",
      "@type": "WebSite",
      name: data.name || "Syed Sohail Portfolio",
      url: data.url || baseUrl,
      description:
        data.description || "Portfolio of Syed Sohail, a Software Developer.",
      author: {
        "@type": "Person",
        name: data.authorName || "Syed Sohail",
      },
      potentialAction: {
        "@type": "SearchAction",
